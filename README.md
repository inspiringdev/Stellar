...
Here is the complete, properly formatted README.md combining the structure you provided with the correct, updated data and analysis sections from our previous conversation.

***

# STELLAR: LLM-Assisted RSMA Optimisation for LEO Satellite Networks

> Implementation of **"STELLAR: Large Language Model-Assisted Optimization for Satellite Networks with RSMA"**, GLOBECOM 2025.

---

## 📋 Overview

STELLAR is a three-phase evolutionary framework that uses a Large Language Model (LLM) as an intelligent operator to optimise power allocation in LEO satellite networks with Rate-Splitting Multiple Access (RSMA).
...
The framework jointly optimises the transmit power of the **common stream** and each **private stream** to maximise the RSMA sum rate across all ground stations, subject to total power and minimum per-user rate constraints.

---

## 🛠️ Key Improvements in this Implementation
Unlike a basic reproduction, this version includes specific enhancements to ensure stability and convergence of the baselines:
- **State Normalization**: PPO observations are normalized to zero mean and unit variance to prevent vanishing gradients.
- **Reward Shaping**: Implements a `-2.0` penalty for constraint violations to guide the AI away from illegal power allocations.
- **Action Mapping**: Uses **Softmax** activation for power allocation to ensure the total power budget is always exactly 100% utilized.
...
│   ├── rsma.py                      ← RSMA / SDMA rate computation (Eqs 6–12)
│   ├── stellar.py                   ← STELLAR algorithm (Algorithm 1)
│   ├── llm_operator.py              ← Dual API LLM operator (Fig 1 prompt)
│   └── baselines.py                 ← Random search + Vanilla PPO (PyTorch)
├── experiments/
│   ├── fig2_convergence.py          ← reproduce Fig 2
│   ├── fig3_rate_vs_params.py       ← reproduce Fig 3a & 3b
...
# 3. Install PyTorch for the Vanilla PPO baseline
pip install torch
```
## 🔑 API Key Setup
